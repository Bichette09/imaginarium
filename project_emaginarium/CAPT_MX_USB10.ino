...
Serial.begin(2000000);
  ...
  pinMode(TRIGGER_PIN9, OUTPUT);
  digitalWrite(TRIGGER_PIN9, LOW); // La broche TRIGGER doit être à LOW au repos
  pinMode(ECHO_PIN9, INPUT);
  /* Initialise les broches CAPT5*/
  pinMode(TRIGGER_PIN10, OUTPUT);
  digitalWrite(TRIGGER_PIN10, LOW); // La broche TRIGGER doit être à LOW au repos
...
/* 2. Mesure le temps entre l'envoi de l'impulsion ultrasonique et son écho (si il existe) */
  long measure1 = pulseIn(ECHO_PIN1, HIGH, MEASURE_TIMEOUT_1m5);
...
/* 2. Mesure le temps entre l'envoi de l'impulsion ultrasonique et son écho (si il existe) */
  long measure2 = pulseIn(ECHO_PIN2, HIGH, MEASURE_TIMEOUT_1m5);
...
// CAPT9 :  RANGE 1.5m
/* 1. Lance une mesure de distance en envoyant une impulsion HIGH de 10µs sur la broche TRIGGER */
  digitalWrite(TRIGGER_PIN9, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIGGER_PIN9, LOW);
/* 2. Mesure le temps entre l'envoi de l'impulsion ultrasonique et son écho (si il existe) */
  long measure9 = pulseIn(ECHO_PIN9, HIGH, MEASURE_TIMEOUT_1m5);

  // CAPT10 : RANGE 1.5m
/* 1. Lance une mesure de distance en envoyant une impulsion HIGH de 10µs sur la broche TRIGGER */
  digitalWrite(TRIGGER_PIN10, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIGGER_PIN10, LOW);
/* 2. Mesure le temps entre l'envoi de l'impulsion ultrasonique et son écho (si il existe) */
  long measure10 = pulseIn(ECHO_PIN10, HIGH, MEASURE_TIMEOUT_1m5);
  
  float distance1_cm = (measure1 / 2.0 * SOUND_SPEED) / 10;
  float distance2_cm = (measure2 / 2.0 * SOUND_SPEED) / 10;   
  ...
  float distance9_cm = (measure9 / 2.0 * SOUND_SPEED) / 10;
  float distance10_cm = (measure10 / 2.0 * SOUND_SPEED) / 10;
